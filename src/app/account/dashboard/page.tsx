...
import AddStudentWizard from "@/components/account/AddStudentWizard";
...
function ParentDashboardContent() {
  ...
  const handleTabChange = (tab: "students" | "entries") => {
    router.push(`/account/dashboard?tab=${tab}`);
  };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data.code === "SETUP_REQUIRED") {
          const tokenRes = await fetch("/api/account/generate-setup-token", {
            method: "POST",
          });
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/account/students", {
        method: "POST",
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      {selectedStudentId && (
                        <button
                          onClick={() => router.push("/account/dashboard?tab=entries")}
