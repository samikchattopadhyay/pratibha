...
export async function POST(req: Request) {
  try {
    ...
    const { email, password, name, phone, address, city, state, postalCode, country } = body;

    // Validate fields
    if (!email || !password || !name || !phone || !address || !city || !state || !postalCode) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const parent = await tx.parent.create({
        data: {
          ...
          city,
          state,
          preferredState: body.preferredState || state,
          postalCode,
